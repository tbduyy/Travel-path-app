...
import { Cloud, CloudRain, Sun, CloudLightning, CloudSnow, MapPin } from "lucide-react";
...
export default function WeatherWidget() {
    ...
    const fetchWeather = () => {
        ...
        const getWeatherData = async (lat: number, lon: number, locationFallback?: string) => {
            try {
                // 1. Get Weather
                ...
                if (!locationName) {
                    ...
                    locationName = geoJson.address.city || geoJson.address.town || geoJson.address.municipality || "My Location";
                }
    ...
    const getWeatherInfo = (code: number) => {
        if (code === 0 || code === 1) return { icon: <Sun className="w-12 h-12 text-yellow-400" />, label: "Sunny" };
        if (code >= 2 && code <= 48) return { icon: <Cloud className="w-12 h-12 text-gray-200" />, label: "Cloudy" };
        if (code >= 51 && code <= 67) return { icon: <CloudRain className="w-12 h-12 text-blue-200" />, label: "Rainy" };
        if (code >= 71 && code <= 77) return { icon: <CloudSnow className="w-12 h-12 text-white" />, label: "Snowy" };
        if (code >= 95) return { icon: <CloudLightning className="w-12 h-12 text-yellow-200" />, label: "Stormy" };
        return { icon: <Sun className="w-12 h-12 text-yellow-400" />, label: "Clear" };
    };
        ...
        <div className="relative w-64 h-64 md:w-80 md:h-80 rounded-[2.5rem] bg-gradient-to-b from-[#2E80C3] to-[#1A5F95] p-6 text-white shadow-2xl overflow-hidden flex flex-col justify-between group hover:scale-105 transition-transform duration-300 font-sans">
            ...
            <div className="relative z-10 flex flex-col items-center justify-center -mt-2">
                <div className="relative">
                    ...
                    <span className="absolute top-2 -right-4 text-3xl font-light opacity-80">°</span>
                ...
                {[0, 1, 2, 3].map((i) => (
                    <div key={i} className="flex flex-col items-center gap-0.5">
                        ...
                        <div className="opacity-90 scale-75">
                            {i % 2 === 0 ? <Cloud className="w-4 h-4" /> : <CloudRain className="w-4 h-4" />}
                        </div>
                        <span className="text-xs font-semibold">{data?.temperature! - i}°</span>
                    </div>
                ))}
