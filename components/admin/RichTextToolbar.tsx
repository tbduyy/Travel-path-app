...
export function markdownToHtml(md: string): string {
    ...
    // Font
    html = html.replace(/\{font:([^}]+)\}(.+?)\{\/font\}/g, '<span style="font-family:$1">$2</span>')
    // Links
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" style="color:#2E968C;text-decoration:underline" target="_blank">$1</a>')
    ...
    html = blocks.map(b => {
        ...
        if (!trimmed) return ''
        if (/^<(h[1-6]|blockquote|hr|ul|ol|img|div)/.test(trimmed)) return trimmed
    ...
    function walk(node: Node): string {
        ...
        switch (tag) {
            ...
            case 'hr':
                return '\n---\n'
            case 'ul':
                return '\n' + Array.from(el.querySelectorAll(':scope > li')).map(li => `- ${walk(li)}`).join('\n') + '\n'
            case 'ol':
                return '\n' + Array.from(el.querySelectorAll(':scope > li')).map((li, i) => `${i + 1}. ${walk(li)}`).join('\n') + '\n'
            case 'li':
                return children
            case 'a': {
...
function Sep() {
    return <div className="w-px h-6 bg-gray-200 mx-0.5 sm:mx-1 self-center shrink-0" />
}
...
export default function WysiwygEditor({ initialMarkdown, onMarkdownChange, canUpload = false }: Props) {
    ...
    const handleFont = (ff: string) => {
        if (!ff) { setShowFontMenu(false); return }
        exec('fontName', ff)
        setShowFontMenu(false)
    }
        ...
        <div className="space-y-0">
            ...
            <div className="flex flex-wrap items-center gap-0.5 p-1.5 sm:p-2 bg-gray-50 border border-gray-200 rounded-t-xl sticky top-0 z-20">
                ...
                <div className="relative" ref={fontMenuRef}>
                    <ToolbarBtn onClick={() => setShowFontMenu(!showFontMenu)} title="Đổi font chữ" active={showFontMenu}>
                        <div className="flex items-center gap-1">
                            <Type size={16} />
                            <span className="text-[11px] hidden sm:inline font-medium">Font</span>
                        </div>
                    </ToolbarBtn>
                    {showFontMenu && (
                        <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-50 py-1 w-56 animate-in fade-in slide-in-from-top-2 duration-150">
                            {FONTS.map((f) => (
                                <button
                                    key={f.value || 'default'}
                                    type="button"
                                    onMouseDown={(e) => { e.preventDefault(); handleFont(f.value) }}
                                    className="w-full text-left px-4 py-2.5 hover:bg-secondary text-sm transition-colors flex items-center gap-3"
                                >
                                    <span className="text-base w-8 text-center" style={{ fontFamily: f.value || 'inherit' }}>Aa</span>
                ...
                {/* Format */}
                <ToolbarBtn onClick={() => exec('bold')} title="In đậm (Ctrl+B)" active={fmt.bold}>
                    <Bold size={16} strokeWidth={2.5} />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('italic')} title="In nghiêng (Ctrl+I)" active={fmt.italic}>
                    <Italic size={16} />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('underline')} title="Gạch chân (Ctrl+U)" active={fmt.underline}>
                    <Underline size={16} />
                </ToolbarBtn>
                ...
                {/* Headings */}
                <ToolbarBtn onClick={() => exec('formatBlock', 'H2')} title="Tiêu đề lớn">
                    <Heading1 size={16} />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('formatBlock', 'H3')} title="Tiêu đề phụ">
                    <Heading2 size={16} />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('formatBlock', 'P')} title="Đoạn văn bình thường">
                ...
                {/* Lists */}
                <ToolbarBtn onClick={() => exec('insertUnorderedList')} title="Danh sách chấm">
                    <List size={16} />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('insertOrderedList')} title="Danh sách số">
                    <ListOrdered size={16} />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('formatBlock', 'BLOCKQUOTE')} title="Trích dẫn">
                    <Quote size={16} />
                </ToolbarBtn>
                ...
                {/* Insert */}
                <ToolbarBtn onClick={() => exec('insertHorizontalRule')} title="Đường kẻ ngang">
                    <Minus size={16} />
                </ToolbarBtn>
            ...
            <p className="text-xs text-gray-400 mt-1.5 flex gap-3 flex-wrap">
                <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-[10px]">Ctrl+B</kbd> Đậm</span>
                <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-[10px]">Ctrl+I</kbd> Nghiêng</span>
                <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-[10px]">Ctrl+U</kbd> Gạch chân</span>
                <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-[10px]">Ctrl+Z</kbd> Hoàn tác</span>
            </p>
            ...
            {showLinkModal && (
                <div className="fixed inset-0 bg-black/40 z-100 flex items-center justify-center p-4 animate-in fade-in duration-150">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md space-y-4 animate-in zoom-in-95 duration-200">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Văn bản hiển thị</label>
                            <input
                                type="text"
                                value={linkText}
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Địa chỉ URL</label>
                            <input
                                ref={linkUrlRef}
                                type="url"
                                value={linkUrl}
