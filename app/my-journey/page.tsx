...
const getWeather = (dateStr: string, destination: string = "") => {
    ...
    let tempModifier = 0;
    if (destination.includes("Đà Lạt") || destination.includes("Da Lat")) tempModifier = -8;
...
function MyJourneyContent() {
    ...
    const currentWeather = React.useMemo(() => getWeather(selectedDayDate, destination || ""), [selectedDayDate, destination]);
        ...
        const fetchMyTrips = async () => {
            ...
            try {
                ...
                if (result.success && result.data && result.data.length > 0) {
                    ...
                    const diffDays = Math.ceil(Math.abs(tEnd.getTime() - tStart.getTime()) / (1000 * 60 * 60 * 24)) + 1;
                    ...
                    latestTrip.items.forEach((item: any) => {
                        ...
                        let period = 'morning';
                        const startHour = item.startTime ? parseInt(item.startTime.split(':')[0]) : 9;
                        if (startHour >= 12) period = 'afternoon';
                        if (startHour >= 18) period = 'evening';

                        if (!dbActivities[day]) dbActivities[day] = { morning: [], afternoon: [], evening: [] };

                        dbActivities[day][period].push({
                            id: item.id,
                            title: item.title,
                            time: (item.startTime && item.endTime) ? `${item.startTime} - ${item.endTime}` : (item.transitDuration || ""),
                            place: item.place,
                            period: period
                        });
                    });
    ...
    const mapMarkers = React.useMemo(() => {
        ...
        return orderedItems.filter(item => item.place).map((item, index) => ({
            id: item.place.id,
            ...
            <div className="flex-1 w-full max-w-[1500px] mx-auto p-4 md:p-6 pb-24">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    ...
                    <div className="flex bg-white/50 p-1 rounded-full border border-[#1B4D3E]/10">
                        <button
                            onClick={() => { setMainTab('my_trips'); setSubTab('upcoming'); }}
                        ...
                        <button
                            onClick={() => { setMainTab('my_plans'); setSubTab('favorite'); }}
                ...
                <div className="bg-white/40 rounded-[40px] border border-[#1B4D3E]/5 min-h-[600px] p-1 relative">
                    ...
                    {mainTab === 'my_trips' && subTab === 'upcoming' && (
                        <div className="h-[800px] p-4"> {/* Fixed height for scroll area */}
                            <ItineraryView
                                ...
                                onPreviousDay={() => selectedDay > 1 && setSelectedDay(d => d - 1)}
                                onNextDay={() => selectedDay < tripDays && setSelectedDay(d => d + 1)}
                                onEditActivity={() => { }}
                                onDeleteActivity={() => { }}
                                onAddActivity={() => { }}
                                isReadOnly={false}
                    ...
                    {mainTab === 'my_plans' && subTab === 'reference' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer">
                                    <div className="relative h-48 rounded-xl bg-gray-200 mb-4 overflow-hidden">
                                        <Image src={`/placeholder.jpg`} alt="Trip" fill className="object-cover" />
                                        <div className="absolute top-2 right-2 bg-white/90 px-3 py-1 rounded-full text-xs font-bold text-[#1B4D3E]">
                                            4N3Đ
                                        </div>
                                    </div>
                                    <h3 className="font-bold text-lg text-[#1B4D3E] mb-1">Khám phá Nha Trang - Hòn Ngọc Biển Đông</h3>
