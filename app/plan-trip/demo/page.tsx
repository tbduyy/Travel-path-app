...
function DemoContent() {
    ...
    // Derived Data & State
    const [showPaymentRequirementModal, setShowPaymentRequirementModal] = useState(false);
    ...
    const handleProceedToPayment = () => {
        ...
        saveTripToSupabase(`/plan-trip/payment?${params.toString()}`);
    };

    const handleSkipPayment = () => {
        const params = new URLSearchParams(searchParams.toString());
        saveTripToSupabase(`/my-journey?${params.toString()}`);
    };
            ...
            {showPaymentRequirementModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-[32px] p-8 max-w-md w-full shadow-2xl relative animate-in zoom-in-95">
                        ...
                        <h3 className="text-xl font-bold text-[#1B4D3E] mb-3 text-center">Thanh toán dịch vụ</h3>

                        {daysUntilTrip > 5 ? (
                            <>
                                <p className="text-gray-600 text-sm mb-6 text-center leading-relaxed">
                                    Để đảm bảo dịch vụ tốt nhất, bạn cần hoàn tất thanh toán trước ngày khởi hành <strong>5 ngày</strong>.<br /><br />
                                    Hiện tại còn <strong className="text-[#2E968C]">{daysUntilTrip} ngày</strong> nữa mới đến ngày đi. Bạn có muốn thanh toán ngay bây giờ không?
                                </p>
                            ...
                            <>
                                <p className="text-red-600 text-sm mb-6 text-center leading-relaxed bg-red-50 p-4 rounded-xl border border-red-100">
                                    <strong>Chú ý:</strong> Thời gian đến ngày khởi hành chỉ còn <strong>{daysUntilTrip} ngày</strong> (dưới 5 ngày).<br />
                                    Bạn <strong>bắt buộc phải thanh toán ngay</strong> để giữ chỗ và lưu lịch trình này.
                                </p>
                                <div className="flex flex-col gap-3">
                                    <button
                                        onClick={handleProceedToPayment}
            ...
            <div className="flex-1 w-full max-w-[1500px] mx-auto p-4 md:p-6 pb-24 md:pb-6 flex flex-col gap-8">
                ...
                <div className="w-full aspect-video bg-black/20 rounded-[32px] overflow-hidden relative shadow-2xl group cursor-pointer border-4 border-white/50">
                    ...
                    <div className="absolute bottom-6 right-6 w-16 h-16 bg-white rounded-full shadow-xl border-4 border-[#41C7D6] overflow-hidden flex items-center justify-center">
                        <Image src="/chatbot-icon.png" alt="Bot" fill className="object-cover scale-150 object-right" />
                    </div>
                </div>
