...
function PlacesContent() {
  ...
  useEffect(() => {
    setStep("places");
    completeStep("search"); // Mark search as complete when we get here
  ...
  // URL param for backward compatibility only
  const destinationParam = searchParams.get("destination");

  // Use store destination as primary, URL param as fallback
  const effectiveDestination = storeDestination || destinationParam || "";

  useEffect(() => {
    // Set destination name from store or URL
    setDestinationName(effectiveDestination);

    if (effectiveDestination) {
      performSearch(effectiveDestination);
    }

    // Sync URL params to store only if store is empty (backward compatibility)
    if (destinationParam && !storeDestination) {
      setTripInfo({
    ...
    // Load pre-selected from URL if store is empty (backward compatibility)
    const existingPlaces = searchParams.get("places");
    if (existingPlaces && selectedPlaceIds.length === 0) {
      const ids = existingPlaces.split(",").filter(Boolean);
      setStorePlaces(ids);
    }
  }, [effectiveDestination, destinationParam, storeDestination]);
  ...
  const toggleSelectPlace = (e: React.MouseEvent, place: any) => {
    ...
    // Use Zustand store's togglePlace
    togglePlace(placeId, place);
  ...
  const handleContinue = () => {
    ...
    completeStep("places"); // Mark places step as complete
    // Navigate using Zustand store - no URL params needed
    router.push(`/plan-trip/hotels`);
  };
                  ...
                  >
                    ...
                    <div className="px-1 flex flex-col gap-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {(place.metadata?.tags || ["#ThamQuan"])
                          .slice(0, 2)
                          .map((tag: string, i: number) => (
                            <span
                              key={i}
                              className="text-[10px] uppercase font-bold text-[#1B4D3E]/60 bg-[#1B4D3E]/5 px-2 py-1 rounded-md"
                            >
                              {tag.replace("#", "")}
                            </span>
                          ))}
