...
const getWeather = (dateStr: string, destination: string = "") => {
    ...
    const isToday = dateStr === new Date().toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' });
...
function TripsContent() {
    ...
    if (startDateParam) {
        ...
        dateRangeStr = start.toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' });
        if (endDateParam) {
    ...
    const [step, setStep] = useState(1); // 1 = List, 2 = Schedule
    ...
    const [activities, setActivities] = useState<Record<number, Record<string, any[]>>>({});
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                // Also fetch hotels
                const hotelResult = await searchPlaces({ destination: destination || "", type: 'HOTEL' });

                let fetchedPlaces = [...(result.data || []), ...(hotelResult.data || [])];
    ...
    const handleAddActivity = (activityData: {
        ...
        place?: { id: string; name: string; image: string | null; address: string | null };
        startTime?: string;
        endTime?: string;
    }) => {
        if (editingActivity) {
            ...
            setActivities(prev => {
                ...
                if (editingActivity.period !== activityData.period) {
                    const oldPeriodActivities = (dayActivities[editingActivity.period] || []).filter(a => a.id !== editingActivity.id);
                    const newPeriodActivities = [...(dayActivities[activityData.period] || []), { ...editingActivity, ...activityData }];
                    ...
                    const periodActivities = (dayActivities[activityData.period] || []).map(a =>
        ...
        if (step === 1) {
            ...
            try {
                ...
                const requestBody = {
                    ...
                    wishlist_spots: [],
                    budget: budgetParam ? parseFloat(budgetParam.replace(/[^0-9]/g, "")) : 5000000,
                    num_people: parseInt(people || "2"),
                    travel_style: "cultural",
                    start_date: startDateParam || new Date().toISOString().split("T")[0],
                    end_date: endDateParam || new Date(Date.now() + 2 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
                    start_time: "08:00",
                    end_time: "21:00",
                ...
                aiData.schedule?.forEach((daySchedule: any, dayIdx: number) => {
                    ...
                    daySchedule.activities?.forEach((act: any) => {
                        ...
                        newActivities[dayNum][period].push({
                            ...
                            time: act.time_slot,
                            cost: act.estimated_cost ? `${act.estimated_cost.toLocaleString()} VND` : undefined,
                            period,
                            place: {
                ...
                alert("Có lỗi khi tạo lịch trình AI. Bạn có thể thêm hoạt động thủ công!");
            } finally {
    ...
    const forecastDays = React.useMemo(() => {
        ...
        if (startDateParam) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                // Get Day Name (T2, T3...)
                const dayName = d.toLocaleDateString('vi-VN', { weekday: 'short' }).replace("Th ", "T").replace("CN", "CN");
    ...
    if (hotelId) {
        selectedHotel = allPlaces.find(p => p.id === hotelId);
        if (!selectedHotel) {
    ...
    const allSelectedLocations = selectedHotel ? [...selectedPlaces, selectedHotel] : selectedPlaces; // Combined for activity modal
    ...
    const availableHotels = allPlaces.filter(p => !placeIds.includes(p.id) && p.type === 'HOTEL');
    ...
    const availableAttractions = availablePlaces.filter(p => p.type === 'ATTRACTION' && extraPlaceIds.includes(p.id));
    const availableRestaurants = availablePlaces.filter(p => p.type === 'RESTAURANT' && extraPlaceIds.includes(p.id));
    ...
    }, [step, selectedDay, activities, selectedPlaces, availablePlaces, viewedPlace]);
    ...
    return (
        <div className="min-h-screen flex flex-col font-sans text-[#1B4D3E] bg-[#BBD9D9] overflow-hidden">
            ...
            <div className="flex-1 w-full max-w-[1500px] mx-auto p-4 md:p-6 pb-24 md:pb-6 h-[calc(100vh-80px)] flex flex-col">
                {/* Header Section */}
                <div className="flex flex-col mb-4 gap-6 shrink-0">
                    ...
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => setStep(1)}
                            className={`px-8 py-2.5 rounded-full font-bold text-lg transition-all border-2 ${step === 1
                        ...
                        <button
                            onClick={() => setStep(2)}
                            className={`px-8 py-2.5 rounded-full font-bold text-lg transition-all border-2 ${step === 2
                                ? "bg-[#1B4D3E] text-white border-[#1B4D3E] shadow-md"
                                : "bg-transparent border-[#1B4D3E] text-[#1B4D3E] hover:bg-[#1B4D3E]/5"
                                }`}
                        >
                ...
                <div className="flex-1 min-h-0 relative">
                            ...
                            <div className="h-full flex flex-col">
                                <div className="bg-[#F0F5F5] p-6 rounded-[32px] border border-[#1B4D3E]/5 shadow-sm h-full flex flex-col overflow-hidden relative">
                                    ...
                                    <div className="overflow-y-auto pr-2 flex-1 scrollbar-thin scrollbar-thumb-[#1B4D3E]/20 hover:scrollbar-thumb-[#1B4D3E]/40 space-y-8 pb-10">
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex-1 flex flex-col justify-start py-1 min-w-0">
                                                                <div className="flex justify-between items-start gap-2">
                                                                    <h3 className="font-bold text-[#1B4D3E] text-lg leading-tight line-clamp-2">{place.name}</h3>
                                                                ...
                                                                <p className="text-xs text-gray-500 font-medium mt-1 mb-1 truncate">
                                                                    {place.type === 'ATTRACTION' ? 'Điểm tham quan' : 'Nhà hàng'} - Thời lượng: {place.duration || '1 - 2 giờ'}
                                                                </p>

                                                                {/* Google Rating Mock */}
                                                                <div className="flex items-center gap-1 mb-3">
                                                                    <div className="w-4 h-4 relative">
                                                                        <Image src="/google-icon.png" alt="G" fill />
                                                                    </div>
                                                                ...
                                                                <div className="flex flex-col gap-2 mt-auto">
                                                                    <div className="inline-flex items-center gap-2 bg-[#CFE0E0] px-3 py-1.5 rounded-full w-fit">
                                                                        <svg className="w-3.5 h-3.5 text-[#5A7A7A]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                                                        <span className="text-[10px] font-bold text-[#3C5757]">Cách Khách sạn: {place.metadata?.distance || (Math.random() * 5 + 1).toFixed(1) + " km"}</span>
                                                                    </div>
                                                                    <div className="inline-flex items-center gap-2 bg-[#CFE0E0] px-3 py-1.5 rounded-full w-fit">
                                                                        <svg className="w-3.5 h-3.5 text-[#5A7A7A]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                                        <span className="text-[10px] font-bold text-[#3C5757] truncate max-w-[150px]">{place.price || "Miễn phí"}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <div className="flex items-center gap-2 mb-2 sticky top-0 bg-[#F0F5F5] z-10 py-2">
                                                    ...
                                                    <div className="flex items-baseline justify-between w-full">
                                                        ...
                                                        <div className="flex text-xs font-bold gap-3 text-gray-400">
                                                            <span className="text-[#2E968C] flex items-center gap-1 cursor-pointer">◉ Hot</span>
                                                            <span className="cursor-pointer hover:text-[#1B4D3E]">○ Local</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                    ...
                                                    {availableRestaurants.map((place) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex-1 flex flex-col justify-start py-1 min-w-0">
                                                                ...
                                                                <p className="text-xs text-gray-500 font-medium mt-1 mb-1 truncate">
                                                                    Nhà hàng - Thời lượng: {place.duration || '1 - 2 giờ'}
                                                                </p>
                                        ...
                                        {availableAttractions.length === 0 && availableRestaurants.length === 0 && (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-[60%_40%] gap-6 h-full">
                            ...
                            <div className="flex flex-col gap-4 h-full">
                                ...
                                {isGenerating && (
                                    <div className="flex flex-col items-center justify-center py-12 bg-white rounded-2xl shadow-sm">
                                        ...
                                        <p className="text-[#1B4D3E] font-bold text-lg">AI đang tạo lịch trình...</p>
                                                ...
                                                {(activities[selectedDay]['morning']).map((item, idx) => (
                                                    <ActivityCard
                                                        ...
                                                        onDelete={() => {
                                                            ...
                                                            newActivities[selectedDay]['morning'] = newActivities[selectedDay]['morning'].filter(i => i.id !== item.id);
                                                            setActivities(newActivities);
                                                        }}
                                                    />
                                                ))}
                                            ...
                                            <div className="space-y-4">
                                                {(activities[selectedDay]['afternoon']).map((item, idx) => (
                                                    <ActivityCard
                                                        ...
                                                        onDelete={() => {
                                                            const newActivities = { ...activities };
                                                            newActivities[selectedDay]['afternoon'] = newActivities[selectedDay]['afternoon'].filter(i => i.id !== item.id);
                                                            setActivities(newActivities);
                                                        }}
                                                    />
                                                ))}
                                            ...
                                            <div className="space-y-4">
                                                {(activities[selectedDay]['evening']).map((item, idx) => (
                                                    <ActivityCard
                                                        ...
                                                        onDelete={() => {
                                                            const newActivities = { ...activities };
                                                            newActivities[selectedDay]['evening'] = newActivities[selectedDay]['evening'].filter(i => i.id !== item.id);
                                                            setActivities(newActivities);
                                                        }}
                                                    />
                                                ))}
                            ...
                            <div className="hidden lg:flex flex-col gap-6 h-full">
                                ...
                                <div className="flex-1 rounded-[32px] overflow-hidden shadow-lg border border-[#1B4D3E]/10 bg-[#E0E8E8] relative z-10">
                                    <MapComponent
                                        markers={mapMarkers}
                                        showRoutes={step === 2} // Show routes only in detailed view
                                        sequentialRoute={step === 2}
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    {/* DETAIL OVERLAY */}
                    {viewedPlace && (
                        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setViewedPlace(null)}>
                            <div
                            ...
                            >
                                ...
                                <div className="w-full aspect-video rounded-2xl overflow-hidden relative mb-4">
                                    <Image src={viewedPlace.image || "/placeholder.jpg"} alt={viewedPlace.name} fill className="object-cover" />
                                </div>
                                <h2 className="text-2xl font-black text-[#1B4D3E] mb-2">{viewedPlace.name}</h2>
                                <p className="text-gray-600 mb-4 text-sm leading-relaxed">{viewedPlace.description || "Mô tả đang cập nhật..."}</p>

                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div className="bg-gray-50 p-3 rounded-xl">
                                        <p className="text-xs text-gray-400 font-bold uppercase">Giờ mở cửa</p>
                                        <p className="text-[#1B4D3E] font-bold">{viewedPlace.duration || "08:00 - 17:00"}</p>
                                    </div>
                                    <div className="bg-gray-50 p-3 rounded-xl">
                                        <p className="text-xs text-gray-400 font-bold uppercase">Giá vé</p>
                                        <p className="text-[#1B4D3E] font-bold">{viewedPlace.price || "Miễn phí"}</p>
                                    </div>
                                </div>
                ...
                <div className="fixed bottom-0 left-0 right-0 p-6 z-40 bg-gradient-to-t from-[#BBD9D9] to-transparent pointer-events-none">
                    <div className="max-w-[1400px] mx-auto flex justify-end pointer-events-auto">
                        ...
                        >
                            <span>{step === 1 ? "Tiếp tục" : "Hoàn tất"}</span>
...
export default function TripsPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-[#BBD9D9] flex items-center justify-center text-[#1B4D3E] font-bold">Đang tải...</div>}>
            <TripsContent />
        </Suspense>
    );
}
